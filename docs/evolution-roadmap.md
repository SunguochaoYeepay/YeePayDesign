...
| 里程碑 | 状态 | 已完成 | 下一道门槛 |
| --- | --- | --- | --- |
| M0：PC 基线冻结 | 接近完成 | 固定 Shell、Design Token、内容模式、Feature/Page Spec、预览注入、PC Ant 组件契约均已建立。 | 用简单控件验证页确认 OpenDesign 能稳定执行 PC Ant 规则。 |
| M1：端到端验证 | 进行中 | 余额分账规则已完成 Feature Spec、列表/详情/新增/修改页面生成与结构检查。 | 依据 `admin-pc-ant` 契约重新生成，并经产品、UI、前端人工验收。 |
| M2：模式覆盖 | 未开始 | 无。 | 验证详情查看、导入/批处理和轻量编辑案例。 |
| M3：标准化运行 | 未开始 | 已具备初版 QA 清单与规则目录。 | 形成回归样例、资产登记和模式准入流程。 |
| M4：团队推广 | 未开始 | 角色边界已定义。 | 产品可在受控流程中独立发起需求。 |
| M5：多框架扩展 | 规划中 | 平台隔离原则已定义。 | PC 稳定后建立移动端独立契约。 |

当前不应继续扩大页面模板数量。优先完成 Ant 控件验证和余额分账规则复验，确认生成行为已被规则稳定约束。
...
当前状态：基础工程已具备，待 PC Ant 控件验证页通过后冻结。
...
当前状态：结构性生成已通过；Ant 组件契约发布后需重新生成同一案例并完成视觉验收。
...
1. 先用简单 PC Ant 控件验证页检查 Select、Radio、Steps、DatePicker、Tooltip、图标和隐藏状态。
2. 按 `admin-pc-ant` 契约重新生成余额分账规则功能包，完成 M1，并记录问题归因到需求、Spec、模式、模板、组件契约或 Shell。
3. 验证详情与导入/批处理两个真实案例，完成 M2。
4. 将三组通过案例冻结为回归样例，开始 M3 的模式准入与审核流程。
5. 在 M3 稳定后，再讨论目录迁移到 `platforms/admin-pc/` 与移动端框架。
