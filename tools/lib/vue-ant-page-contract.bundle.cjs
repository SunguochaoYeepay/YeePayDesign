...
// tools/lib/vue-ant-page-contract.mjs
var vue_ant_page_contract_exports = {};
__export(vue_ant_page_contract_exports, {
  FORM_TEMPLATE_DEFINITIONS: () => FORM_TEMPLATE_DEFINITIONS,
  FORM_TEMPLATE_IDS: () => FORM_TEMPLATE_IDS,
  SUPPORTED_CAPABILITIES: () => SUPPORTED_CAPABILITIES,
  VUE_ANT_RENDERER_VERSION: () => VUE_ANT_RENDERER_VERSION,
  VUE_ANT_RUNTIME: () => VUE_ANT_RUNTIME,
  pageContentFromVueAntDeclaration: () => pageContentFromVueAntDeclaration,
  readYamlPageSpec: () => readYamlPageSpec,
  resolveFormTemplate: () => resolveFormTemplate,
  toVueAntPageDeclaration: () => toVueAntPageDeclaration,
  validateVueAntPageSpec: () => validateVueAntPageSpec,
  writeVueAntPageContent: () => writeVueAntPageContent
});
module.exports = __toCommonJS(vue_ant_page_contract_exports);
var import_node_fs = __toESM(require("node:fs"), 1);
var import_node_path = __toESM(require("node:path"), 1);
var import_yaml = __toESM(require_dist(), 1);
var VUE_ANT_RUNTIME = "vue-ant";
var VUE_ANT_RENDERER_VERSION = 2;
...
var FORM_TEMPLATE_DEFINITIONS = Object.freeze({
...
var FORM_TEMPLATE_IDS = Object.freeze(Object.keys(FORM_TEMPLATE_DEFINITIONS));
var FORM_CONTROLS = ["static", "input", "select", "radio", "upload"];
var FORM_ACTION_PLACEMENTS = ["control-start", "sticky-end"];
var FORM_FIELD_LAYOUTS = ["single-column", "multi-column"];
var FORM_GRID_COLUMNS = [2, 3, 4];
var FORM_STEP_STATUSES = ["wait", "process", "finish", "error"];
var FORM_REVIEW_COLUMN_FORMATS = ["text", "datetime", "amount"];
var LIST_QUERY_CONTROLS = ["input", "select", "date-range"];
var LIST_COLUMN_FORMATS = ["text", "link", "tag", "status", "amount", "datetime"];
var QUICK_RANGE_KEYS = ["today", "yesterday", "last3days", "last7days", "last30days"];
var TABLE_TOOLS = ["export", "refresh", "settings"];
var LIST_SUMMARY_TYPES = ["count", "active", "sum", "value"];
var STATISTICS_LAYOUTS = ["standard", "rich"];
var STATISTIC_FORMATS = ["amount", "number"];
var STATISTIC_AGGREGATE_OPERATIONS = ["sum", "count"];
var TABLE_SELECTION_MODES = ["multiple"];
var TABLE_SELECTION_QUICK_ACTIONS = ["all-results", "current-page", "invert-current-page", "clear"];
var TABLE_EXPANDABLE_MODES = ["multiple"];
var DETAIL_PRESENTATIONS = ["page", "modal", "drawer"];
var RESULT_PRESENTATIONS = ["page", "inline-state"];
var DETAIL_FIELD_FORMATS = ["text", "status", "amount", "datetime"];
var DETAIL_FIELD_SPANS = [1, 2, 3, "full"];
var DETAIL_STATUS_TONES = ["default", "success", "error", "warning", "processing"];
var DETAIL_TABLE_COLUMN_FORMATS = ["text", "status", "amount", "datetime"];
var RESULT_STATUSES = ["success", "error", "warning", "processing"];
function readYamlPageSpec(file) {
  return import_yaml.default.parse(import_node_fs.default.readFileSync(file, "utf8"));
}
...
function validateField(errors, field, location, controls) {
  ...
  if (field.control === "upload") {
    ...
    if (field.maxCount !== void 0 && (!Number.isInteger(field.maxCount) || field.maxCount < 1)) {
      errors.push(`${location}.maxCount must be a positive integer when supplied.`);
    }
    if (field.buttonLabel !== void 0 && (typeof field.buttonLabel !== "string" || !field.buttonLabel.length)) {
      errors.push(`${location}.buttonLabel must be non-empty text when supplied.`);
    }
    if (field.templateAction !== void 0) validateAction(errors, field.templateAction, `${location}.templateAction`);
  }
}
function validateAction(errors, action, location) {
  ...
  if (action.confirm !== void 0 && typeof action.confirm !== "string") {
    errors.push(`${location}.confirm must be text when supplied.`);
  }
  if (action.danger !== void 0 && typeof action.danger !== "boolean") {
...
function validateStatisticAggregate(errors, aggregate, location) {
  ...
  if (aggregate.field !== void 0 && (typeof aggregate.field !== "string" || !aggregate.field.length)) {
    errors.push(`${location}.field must be text when supplied.`);
  }
  if (aggregate.where !== void 0) {
    ...
    issue(errors, aggregate.where?.equals !== void 0, `${location}.where.equals is required.`);
  }
}
function validateStatistics(errors, spec) {
  const statistics = spec.statistics;
  if (statistics === void 0) return;
  ...
  (statistics.items || []).forEach((item, index) => {
    ...
    if (item.unit !== void 0 && typeof item.unit !== "string") errors.push(`${location}.unit must be text when supplied.`);
    if (item.helpText !== void 0 && typeof item.helpText !== "string") errors.push(`${location}.helpText must be text when supplied.`);
    const hasValue = typeof item.value === "number" && Number.isFinite(item.value);
    const hasAggregate = item.aggregate !== void 0;
    if (hasValue === hasAggregate) errors.push(`${location} must declare exactly one of value or aggregate.`);
    if (item.value !== void 0 && !hasValue) errors.push(`${location}.value must be a finite number.`);
    if (hasAggregate) validateStatisticAggregate(errors, item.aggregate, `${location}.aggregate`);
    if (item.detailItems !== void 0) {
    ...
    if (item.action !== void 0) validateAction(errors, item.action, `${location}.action`);
  });
...
function validateTableSelection(errors, selection) {
  ...
  if (selection.itemLabel !== void 0 && (typeof selection.itemLabel !== "string" || !selection.itemLabel.length)) {
...
function validateChildTableColumn(errors, column, location) {
  ...
  if (column.format !== void 0 && !LIST_COLUMN_FORMATS.includes(column.format)) {
  ...
  if (column.fixed !== void 0) errors.push(`${location}.fixed is not supported in the current child-table renderer.`);
  if (column.align !== void 0 && !["left", "center", "right"].includes(column.align)) {
    errors.push(`${location}.align must be left, center, or right.`);
  }
  if (column.ellipsis !== void 0 && typeof column.ellipsis !== "boolean") {
...
function validateTableExpandable(errors, table) {
  ...
  ["rows", "pagination", "selection", "batchActions", "expandable", "rowActions", "primaryAction", "secondaryActions", "tools"].forEach((key) => {
    if (childTable[key] !== void 0) errors.push(`table.expandable.childTable.${key} is not supported in the current child-table renderer.`);
  });
}
...
function inferredFormTemplateId(spec) {
  ...
  if (form.uploadFlow !== void 0) return "form.import-review-flow";
...
function validateListWorkflow(errors, spec) {
  ...
  if (create !== void 0) {
    ...
    if (create && typeof create === "object") {
      ...
      if (create.width !== void 0 && (!Number.isInteger(create.width) || create.width < 360 || create.width > 720)) errors.push("workflow.createDrawer.width must be an integer from 360 to 720.");
      ...
      ["primaryLabel", "cancelLabel"].forEach((key) => {
        if (form?.[key] !== void 0 && (typeof form[key] !== "string" || !form[key].length)) errors.push(`workflow.createDrawer.form.${key} must be non-empty text when supplied.`);
      });
      ...
      if (addRow && typeof addRow === "object") {
        ...
        Object.entries(addRow.fields || {}).forEach(([key, mapping]) => {
          ...
          if (mapping.from !== void 0 && (typeof mapping.from !== "string" || !formKeys.includes(mapping.from))) errors.push(`${location}.from must reference a workflow.createDrawer.form field.`);
          if (mapping.value !== void 0 && !isWorkflowValue(mapping.value)) errors.push(`${location}.value must be a scalar, null, or array when supplied.`);
          if (mapping.from === void 0 && mapping.value === void 0) errors.push(`${location} requires from or value.`);
          if (mapping.from !== void 0 && mapping.value !== void 0) errors.push(`${location} cannot declare both from and value.`);
        });
        columns.forEach((column) => {
          if (!addRow.fields?.[column.key]) errors.push(`workflow.createDrawer.addRow.fields must map table column ${column.key}.`);
        });
      }
    }
  }
  ...
  if (detail !== void 0) {
    ...
    if (detail && typeof detail === "object") {
      ...
      if (detail.width !== void 0 && (!Number.isInteger(detail.width) || detail.width < 360 || detail.width > 960)) errors.push("workflow.detailDrawer.width must be an integer from 360 to 960.");
      ...
      (detail.groups || []).forEach((group, groupIndex) => {
        ...
        (group.fields || []).forEach((field, fieldIndex) => {
          ...
          if (field.format !== void 0 && !DETAIL_FIELD_FORMATS.includes(field.format)) errors.push(`${fieldLocation}.format must be one of: ${DETAIL_FIELD_FORMATS.join(", ")}.`);
          if (field.span !== void 0 && !DETAIL_FIELD_SPANS.includes(field.span)) errors.push(`${fieldLocation}.span must be one of: ${DETAIL_FIELD_SPANS.join(", ")}.`);
          if (field.format === "status" && field.tone === void 0) errors.push(`${fieldLocation}.tone is required when format=status.`);
          if (field.tone !== void 0 && !DETAIL_STATUS_TONES.includes(field.tone)) errors.push(`${fieldLocation}.tone must be one of: ${DETAIL_STATUS_TONES.join(", ")}.`);
        });
      });
      ...
      if (detail.closeLabel !== void 0 && (typeof detail.closeLabel !== "string" || !detail.closeLabel.length)) errors.push("workflow.detailDrawer.closeLabel must be non-empty text when supplied.");
    }
  }
  if (create === void 0 && detail === void 0) errors.push("workflow requires createDrawer or detailDrawer.");
}
function resolveFormTemplate(spec) {
...
function validateFormModeTabs(errors, form) {
  ...
  (modeTabs.items || []).forEach((item, index) => {
    ...
    if (item.disabledUntil !== void 0) {
      issue(errors, Array.isArray(item.disabledUntil), `${location}.disabledUntil must be an array.`);
      const availableKeys = /* @__PURE__ */ new Set([...baseKeys, ...(item.fields || []).map((field) => field?.key).filter(Boolean)]);
...
function validateFormGroups(errors, form) {
  ...
  groups.forEach((group, index) => {
    ...
    issue(errors, typeof group.key === "string" && group.key.length > 0, `${location}.key is required.`);
    issue(errors, typeof group.title === "string" && group.title.length > 0, `${location}.title is required.`);
    if (group.description !== void 0 && (typeof group.description !== "string" || !group.description.length)) {
    ...
    (group.fields || []).forEach((field, fieldIndex) => {
      ...
      if (field?.span !== void 0 && (!Number.isInteger(field.span) || field.span < 1 || !FORM_GRID_COLUMNS.includes(form.columns) || field.span > form.columns)) {
...
function validateReviewColumn(errors, column, location) {
  ...
  issue(errors, typeof column.key === "string" && column.key.length > 0, `${location}.key is required.`);
  issue(errors, typeof column.label === "string" && column.label.length > 0, `${location}.label is required.`);
  if (column.format !== void 0 && !FORM_REVIEW_COLUMN_FORMATS.includes(column.format)) {
...
function validateUploadFlow(errors, form) {
  ...
  if (review && typeof review === "object") {
    ...
    (review.rows || []).forEach((row, index) => {
      ...
      if (row && typeof row === "object") {
        issue(errors, row[review.rowKey] !== void 0 && row[review.rowKey] !== null && row[review.rowKey] !== "", `form.uploadFlow.review.rows[${index}] must declare ${review.rowKey}.`);
      }
    });
    const rowKeys = (review.rows || []).map((row) => row?.[review.rowKey]).filter((key) => key !== void 0 && key !== null && key !== "");
  ...
  if (result && typeof result === "object") {
    ...
    if (result.subtitle !== void 0 && (typeof result.subtitle !== "string" || !result.subtitle.length)) {
...
function validateForm(errors, spec) {
  ...
  if (form.fields !== void 0) issue(errors, Array.isArray(form.fields), "form.fields must be an array when supplied.");
  (form.fields || []).forEach((field, index) => validateField(errors, field, `form.fields[${index}]`, FORM_CONTROLS));
  if (form.modeTabs !== void 0) validateFormModeTabs(errors, form);
  ...
  if (form.fieldLayout === "single-column" && form.columns !== void 0) errors.push("form.columns is only supported with form.fieldLayout=multi-column.");
  if (!groupsUsed && form.columns !== void 0) errors.push("form.columns requires form.groups.");
  if (groupsUsed && form.modeTabs !== void 0) errors.push("form.groups cannot be combined with form.modeTabs in the current renderer.");
  ...
  if (form.actions?.secondaryActions !== void 0) {
  ...
  if (form.steps !== void 0) {
    ...
    (form.steps || []).forEach((step, index) => {
      ...
      if (step.fieldKeys !== void 0 && !Array.isArray(step.fieldKeys)) {
        errors.push(`${location}.fieldKeys must be an array when supplied.`);
      }
      if (step.description !== void 0 && (typeof step.description !== "string" || !step.description.length)) {
        errors.push(`${location}.description must be non-empty text when supplied.`);
      }
  ...
  } else if (form.confirmation !== void 0) {
  ...
  if (form.submit?.failureSimulation !== void 0) {
  ...
  if (form.submit?.preserveOnError !== void 0 && typeof form.submit.preserveOnError !== "boolean") {
    errors.push("form.submit.preserveOnError must be a boolean when supplied.");
  }
  if (form.submit?.result !== void 0) validateFormSubmitResult(errors, form.submit.result);
  ...
  if (form.uploadFlow !== void 0) validateUploadFlow(errors, form);
  const stepsUsed = Boolean(form.steps?.length);
  const modeTabsUsed = form.modeTabs !== void 0;
  ...
  const uploadFlowUsed = form.uploadFlow !== void 0;
  const submitResultUsed = form.submit?.result !== void 0;
...
function validateDetailField(errors, field, location) {
  ...
  if (field.format !== void 0 && !DETAIL_FIELD_FORMATS.includes(field.format)) {
    errors.push(`${location}.format must be one of: ${DETAIL_FIELD_FORMATS.join(", ")}.`);
  }
  if (field.span !== void 0 && !DETAIL_FIELD_SPANS.includes(field.span)) {
    errors.push(`${location}.span must be one of: ${DETAIL_FIELD_SPANS.join(", ")}.`);
  }
  if (field.tone !== void 0 && !DETAIL_STATUS_TONES.includes(field.tone)) {
    errors.push(`${location}.tone must be one of: ${DETAIL_STATUS_TONES.join(", ")}.`);
  }
  if (field.format === "status" && field.tone === void 0) {
...
function validateDetailTable(errors, table, location) {
  ...
  (table.columns || []).forEach((column, index) => {
    ...
    if (column.format !== void 0 && !DETAIL_TABLE_COLUMN_FORMATS.includes(column.format)) {
    ...
    if (column.format === "status" && column.statusMap === void 0) {
  ...
  (table.rows || []).forEach((row, index) => {
    ...
    if (row && typeof row === "object") {
      issue(errors, row[table.rowKey] !== void 0 && row[table.rowKey] !== null && row[table.rowKey] !== "", `${location}.rows[${index}] must declare ${table.rowKey}.`);
    }
  });
  const rowKeys = (table.rows || []).map((row) => row?.[table.rowKey]).filter((key) => key !== void 0 && key !== null && key !== "");
...
function validateDetailActions(errors, actions, location) {
  ...
  if (actions.primary !== void 0) validateAction(errors, actions.primary, `${location}.primary`);
...
function validateDetail(errors, spec) {
  ...
  if (metrics !== void 0) {
    ...
    (metrics || []).forEach((item, index) => {
      ...
      if (item?.description !== void 0 && (typeof item.description !== "string" || !item.description.length)) {
        errors.push(`${location}.description must be non-empty text when supplied.`);
      }
    });
  }
  ...
  if (presentation === "modal" && (detail.groups || []).some((group) => group?.table !== void 0)) {
  ...
  if (presentation === "modal" && (detail.groups || []).some((group) => group?.description !== void 0)) {
...
function validateResultActions(errors, actions, location = "result.actions", { allowLabelOnly = false } = {}) {
  ...
  const validateResultAction = (action, actionLocation) => {
    ...
    if (action && typeof action === "object") {
      ...
      if (action.key !== void 0 && (typeof action.key !== "string" || !action.key.length)) {
        errors.push(`${actionLocation}.key must be non-empty text when supplied.`);
      }
    }
  };
  if (actions.primary !== void 0) validateResultAction(actions.primary, `${location}.primary`);
  if (actions.secondary !== void 0) {
    issue(errors, Array.isArray(actions.secondary), `${location}.secondary must be an array.`);
...
function validateResult(errors, spec) {
  ...
  const summaryUsed = result.summary !== void 0;
  const feedbackUsed = result.feedback !== void 0;
...
function validateList(errors, spec) {
  ...
  if (query?.fields) {
    query.fields.forEach((field, index) => {
      ...
      if (field.span !== void 0 && (!Number.isInteger(field.span) || field.span < 1 || field.span > 3)) {
        errors.push(`query.fields[${index}].span must be an integer between 1 and 3.`);
      }
      if (field.advanced !== void 0 && typeof field.advanced !== "boolean") {
        errors.push(`query.fields[${index}].advanced must be a boolean.`);
      }
    });
  }
  if (query?.collapsible !== void 0 && ![true, false, "auto"].includes(query.collapsible)) {
    errors.push("query.collapsible must be true, false, or auto.");
  }
  if (query?.collapseThreshold !== void 0 && (!Number.isInteger(query.collapseThreshold) || query.collapseThreshold < 1)) {
    errors.push("query.collapseThreshold must be a positive integer.");
  }
  if (query?.quickRanges !== void 0) {
  ...
  if (query?.defaultQuickRange !== void 0 && !QUICK_RANGE_KEYS.includes(query.defaultQuickRange)) {
  ...
  if (table) {
    ...
    (table.columns || []).forEach((column, index) => {
      ...
      if (column.fixed !== void 0 && !["left", "right"].includes(column.fixed)) {
      ...
      if (column.required !== void 0 && typeof column.required !== "boolean") {
    ...
    if (table.primaryAction !== void 0) validateAction(errors, table.primaryAction, "table.primaryAction");
    if (table.secondaryActions !== void 0) {
    ...
    if (table.selection !== void 0) validateTableSelection(errors, table.selection);
    if (table.batchActions !== void 0) {
    ...
    (table.rows || []).forEach((row, rowIndex) => {
      ...
      issue(errors, row[table.rowKey] !== void 0 && row[table.rowKey] !== null && row[table.rowKey] !== "", `table.rows[${rowIndex}] must declare ${table.rowKey}.`);
      if (row.actions !== void 0) {
      ...
      if (typeof childRowsKey === "string" && childRowsKey.length) {
        ...
        if (Array.isArray(childRows) && typeof childTable?.rowKey === "string" && childTable.rowKey.length) {
          childRows.forEach((childRow, childIndex) => {
            ...
            issue(errors, childRow[childTable.rowKey] !== void 0 && childRow[childTable.rowKey] !== null && childRow[childTable.rowKey] !== "", `table.rows[${rowIndex}].${childRowsKey}[${childIndex}] must declare ${childTable.rowKey}.`);
          });
          const childKeys = childRows.filter((childRow) => childRow && typeof childRow === "object" && childRow[childTable.rowKey] !== void 0 && childRow[childTable.rowKey] !== null && childRow[childTable.rowKey] !== "").map((childRow) => childRow[childTable.rowKey]);
    ...
    const recordKeys = (table.rows || []).filter((row) => row && typeof row === "object" && row[table.rowKey] !== void 0 && row[table.rowKey] !== null && row[table.rowKey] !== "").map((row) => row[table.rowKey]);
    ...
    if (pagination.pageSize !== void 0 && (!Number.isInteger(pagination.pageSize) || pagination.pageSize < 1)) {
      errors.push("table.pagination.pageSize must be a positive integer.");
    }
    if (pagination.total !== void 0 && (!Number.isInteger(pagination.total) || pagination.total < 0)) {
      ...
      if (table.summary?.items !== void 0) {
        ...
        (table.summary.items || []).forEach((item, index) => {
          ...
          if (item?.type === "value") {
            ...
            if (item.field !== void 0 || item.equals !== void 0) {
              errors.push(`table.summary.items[${index}] value must not declare field or equals.`);
            }
          }
          if (item?.field !== void 0 && typeof item.field !== "string") errors.push(`table.summary.items[${index}].field must be text when supplied.`);
        });
      }
    }
    if (table.summaryTemplate !== void 0 && typeof table.summaryTemplate !== "string") {
      errors.push("table.summaryTemplate must be text when supplied.");
    }
    if (table.sectionTitle !== void 0 && (typeof table.sectionTitle !== "string" || !table.sectionTitle.length)) {
...
function validateListCapabilityUsage(errors, spec) {
  ...
  const hasConfirm = (table.rowActions || []).some((action) => action.confirm) || (table.rows || []).some((row) => (row.actions || []).some((action) => action.confirm)) || (table.batchActions || []).some((action) => action.confirm);
  const hasSummaryAmount = (table.summary?.items || []).some((item) => item.type === "sum");
  const hasSummaryCount = Boolean(table.summaryTemplate) || (table.summary?.items || []).some((item) => ["count", "active", "value"].includes(item.type));
  const toolbarUsed = Boolean(table.primaryAction) || Boolean(table.secondaryActions?.length) || Boolean(table.batchActions?.length) || Boolean(table.tools?.length);
  const statisticsUsed = Boolean(spec.statistics);
  const richStatisticsUsed = spec.statistics?.layout === "rich";
  const selectionUsed = table.selection !== void 0;
  const batchActionsUsed = Boolean(table.batchActions?.length);
  const expandableUsed = table.expandable !== void 0;
...
function validateVueAntPageSpec(spec, options = {}) {
  const errors = [];
  ...
  if (family !== "form" && spec.template !== void 0) {
...
function toVueAntPageDeclaration(spec) {
...
function pageContentFromVueAntDeclaration(declaration) {
...
function writeVueAntPageContent(outputFile, declaration) {
  import_node_fs.default.mkdirSync(import_node_path.default.dirname(outputFile), { recursive: true });
  import_node_fs.default.writeFileSync(outputFile, pageContentFromVueAntDeclaration(declaration));
}
...
0 && (module.exports = {
  ...
  VUE_ANT_RENDERER_VERSION,
  VUE_ANT_RUNTIME,
  pageContentFromVueAntDeclaration,
  readYamlPageSpec,
  resolveFormTemplate,
